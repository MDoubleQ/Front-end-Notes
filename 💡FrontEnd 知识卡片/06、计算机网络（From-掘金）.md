![计算机网络面试题.png](E:\coder\09_OneNote\image\0d8a4290ff7b4a9b899da84865960dcetplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
9.1、HTTP（Hypertext Transfer Protocol）和HTTPS（Hypertext Transfer Protocol Secure）都是用于传输数据的应用层协议，它们之间的区别如下：

1. 安全性：HTTPS 在传输数据时会进行 SSL/TLS 加密，因此比 HTTP 更安全，可以防止数据被窃取或篡改。
2. 端口：HTTP 默认使用 80 端口进行传输，而 HTTPS 默认使用 443 端口进行传输。
3. 证书：HTTPS 需要使用数字证书来验证服务器的身份，确保通信双方的安全性，而 HTTP 则没有这个要求。
4. 速度：HTTPS 会因为加密和解密的过程导致传输速度较慢，而 HTTP     没有这个问题。

综上所述，HTTPS 比 HTTP 更加安全，但是传输速度较慢，需要使用数字证书验证服务器的身份。在传输敏感信息时，建议使用 HTTPS 来保证数据安全。

9.2、HTTP和HTTPS协议的主要区别如下：
...
例如下图中的两个请求， 请求一发送了所有的头部字段，第二个请求则只需要发送差异数据，这样可以减少冗余数据，降低开销。 ![img](E:\coder\09_OneNote\image\bda10d6c69e74996b6505777d29b9a74tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
![image.png](E:\coder\09_OneNote\image\4fb5bb2cb1664850b52e32d57af74f2ftplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) **其中：** （1）请求⾏包括：请求⽅法字段、URL字段、HTTP协议版本字段。它们⽤空格分隔。例如，GET /index.html HTTP/1.1。 （2）请求头部:请求头部由关键字/值对组成，每⾏⼀对，关键字和值⽤英⽂冒号“:”分隔

- User-Agent：产⽣请求的浏览器类型。
- Accept：客户端可识别的内容类型列表。
- Host：请求的主机名，允许多个域名同处⼀个IP地址，即虚拟主机。

（3）请求体: post put等请求携带的数据 ![image.png](E:\coder\09_OneNote\image\eacc55d7152149e99730346f1edfc9abtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
![image.png](E:\coder\09_OneNote\image\1b0183eb91ce451aa17bd515d047062dtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
HTTP/3基于UDP协议实现了类似于TCP的多路复用数据流、传输可靠性等功能，这套功能被称为QUIC协议。 ![img](E:\coder\09_OneNote\image\45a0a2ec0ef143b49d79256cea543418tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)

1. 流量控制、传输可靠性功能：QUIC在UDP的基础上增加了一层来保证数据传输可靠性，它提供了数据包重传、拥塞控制、以及其他一些TCP中的特性。
2. 集成TLS加密功能：目前QUIC使用TLS1.3，减少了握手所花费的RTT数。
3. 多路复用：同一物理连接上可以有多个独立的逻辑数据流，实现了数据流的单独传输，解决了TCP的队头阻塞问题。

![img](E:\coder\09_OneNote\image\48df233ce8e541efa165160c169b7a70tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
![img](E:\coder\09_OneNote\image\f756e23ecf3a4d2a80d632b5fa8b0b6dtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
跨域（Cross-Origin Resource Sharing，简称CORS）是指浏览器在同一页面中发起的请求，目标资源所属的域名、端口号、协议等与当前页面的域名、端口号、协议不一致的情况。由于浏览器同源策略（Same-Origin Policy）的限制，跨域请求通常会被浏览器拦截，导致请求失败。

为了解决跨域问题，通常采用以下几种方法：

1. JSONP：通过在页面上动态创建script标签，使用callback参数指定回调函数，服务器将数据作为参数传入回调函数中，从而实现跨域请求。
2. CORS：服务端在响应请求时，设置Access-Control-Allow-Origin头信息，指定可以访问资源的域名。浏览器接收到响应后会检查Access-Control-Allow-Origin头信息，如果该信息包含了当前页面的域名，则允许访问资源。
3. 代理：在同源策略限制下，使用后端代理来转发请求，将跨域请求转化为同源请求，然后再将响应返回给前端。
4. WebSocket：WebSocket协议不受同源策略限制，可以在客户端与服务端之间建立双向通信的连接，从而实现跨域通信。

证明解决跨域：如果一个网站的页面和资源都在同一个域名下，那么在访问时是不需要跨域解决方案的。因此，可以通过查看浏览器控制台中的网络请求，或者通过查看请求和响应头中的Origin和Access-Control-Allow-Origin信息，来验证是否存在跨域问题，以及是否已经解决跨域问题。如果跨域问题得到了解决，请求会得到成功的响应。

## 二、HTTPS协议

### 1. 什么是HTTPS协议？

超文本传输安全协议（Hypertext Transfer Protocol Secure，简称：HTTPS）是一种通过计算机网络进行安全通信的传输协议。HTTPS经由HTTP进行通信，利用SSL/TLS来加密数据包。HTTPS的主要目的是提供对网站服务器的身份认证，保护交换数据的隐私与完整性。 ![img](E:\coder\09_OneNote\image\10885a9d4d574d7caf3fee1416f623catplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) HTTP协议采用**明文传输**信息，存在**信息窃听**、**信息篡改**和**信息劫持**的风险，而协议TLS/SSL具有**身份验证**、**信息加密**和**完整性校验**的功能，可以避免此类问题发生。
...
![img](E:\coder\09_OneNote\image\5696ee8ccb0d44b08b812a7c964695b7tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
这个方法最要的是认证中心的可靠性，一般浏览器里会内置一些顶层的认证中心的证书，相当于我们自动信任了他们，只有这样才能保证数据的安全。 ![img](E:\coder\09_OneNote\image\90da1f506e7040aaba5e1536c1f6c373tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
`ISO`为了更好的使网络应用更为普及，推出了`OSI`参考模型。 ![img](E:\coder\09_OneNote\image\c1e8e168d9f249788c74c5b50e0528e2tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
`TCP/IP`五层协议和`OSI`的七层协议对应关系如下： ![img](E:\coder\09_OneNote\image\48cd77d608714c298366f4dac77f33fbtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
在每一层都工作着不同的设备，比如我们常用的交换机就工作在数据链路层的，一般的路由器是工作在网络层的。 ![img](E:\coder\09_OneNote\image\8a89b9ddb9d541a880fee0dccd1c03e9tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 在每一层实现的协议也各不同，即每一层的服务也不同，下图列出了每层主要的传输协议： ![img](E:\coder\09_OneNote\image\2db9226a6cdb4ee48daecd9bc4181b64tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)

同样，`TCP/IP`五层协议的通信方式也是对等通信： ![image.png](E:\coder\09_OneNote\image\2c0d79ff592242a8af294ce32177c8e3tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
**5）头部开销小，传输数据报文时是很高效的。** ![img](E:\coder\09_OneNote\image\14d246d14aac47919989fc620fbb17ddtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
- 无论是在慢开始阶段还是在拥塞避免阶段，只要发送方判断网络出现拥塞，就把慢开始门限设置为出现拥塞时的发送窗口大小的一半。然后把拥塞窗口设置为1，执行慢开始算法。如图所示: ![img](E:\coder\09_OneNote\image\06b60b174bb242b498d244002caded1ctplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 其中，判断网络出现拥塞的根据就是没有收到确认，虽然没有收到确认可能是其他原因的分组丢失，但是因为无法判定，所以都当做拥塞来处理。
...
- 考虑到如果网络出现拥塞的话就不会收到好几个重复的确认，所以发送方现在认为网络可能没有出现拥塞。所以此时不执行慢开始算法，而是将cwnd设置为ssthresh的大小，然后执行拥塞避免算法。 ![img](E:\coder\09_OneNote\image\d0f5339da8c741d19f416cac2af18e4ftplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
![img](E:\coder\09_OneNote\image\55a81db348584e57abdd01c9555f1d5ftplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 三次握手（Three-way Handshake）其实就是指建立一个TCP连接时，需要客户端和服务器总共发送3个包。进行三次握手的主要作用就是为了确认双方的接收能力和发送能力是否正常、指定自己的初始化序列号为后面的可靠性传送做准备。实质上其实就是连接服务器指定端口，建立TCP连接，并同步连接双方的序列号和确认号，交换TCP窗口大小信息。
...
![img](E:\coder\09_OneNote\image\a4190d0d324b427cb63154d3f84e47bctplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 刚开始双方都处于 ESTABLISHED 状态，假如是客户端先发起关闭请求。四次挥手的过程如下：
