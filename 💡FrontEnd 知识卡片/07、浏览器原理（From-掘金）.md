![浏览器原理面试题.png](E:\coder\09_OneNote\image\23df95226fe54f26acd8f0d3c62b5d85tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
**Chrome浏览器的架构图**： ![img](E:\coder\09_OneNote\image\83e928a4f95d4ebe91d0881b5b2cf94btplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 从图中可以看出，最新的 Chrome 浏览器包括：
...
浏览器的渲染进程的线程总共有五种： ![img](E:\coder\09_OneNote\image\d6e583f59dc742b9b4e88cf3a3b0f1d4tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) **（1）GUI渲染线程** 负责渲染浏览器页面，解析HTML、CSS，构建DOM树、构建CSSOM树、构建渲染树和绘制页面；当界面需要**重绘**或由于某种操作引发**回流**时，该线程就会执行。
...
打开页面，可以在开发者工具中的 `Application` 看到 Service Worker 已经启动了： ![img](E:\coder\09_OneNote\image\0ef126ed3be8463a8e04d4156b2addd9tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 在 Cache 中也可以发现所需的文件已被缓存： ![img](E:\coder\09_OneNote\image\a223ee3c073d47188ecee7c410ee557ctplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
![业务流程图1.png](E:\coder\09_OneNote\image\3f6837d8d1c74cf2894d8967a20115d9tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 很多网站的资源后面都加了版本号，这样做的目的是：每次升级了 JS 或 CSS 文件后，为了防止浏览器进行缓存，强制改变版本号，客户端浏览器就会重新下载新的 JS 或 CSS 文件 ，以保证用户能够及时获得网站的最新更新。
...
大致过程如图所示： ![img](E:\coder\09_OneNote\image\4d69da20c3f84782948226798effc60atplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
两者区别如图示： ![img](E:\coder\09_OneNote\image\97e92248f2654185808f80b65f3ca0f6tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 正向代理和反向代理的结构是一样的，都是 client-proxy-server 的结构，它们主要的区别就在于中间这个 proxy 是哪一方设置的。在正向代理中，proxy 是 client 设置的，用来隐藏 client；而在反向代理中，proxy 是 server 设置的，用来隐藏 server。
...
![img](E:\coder\09_OneNote\image\426a3bf11f404821abb636690119621btplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
可以把执行栈认为是一个存储函数调用的**栈结构**，遵循先进后出的原则。 ![img](E:\coder\09_OneNote\image\391e7d7e880f42d59809dbc16af5d7bftplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 当开始执行 JS 代码时，根据先进后出的原则，后执行的函数会先弹出栈，可以看到，`foo` 函数后执行，当执行完毕后就从栈中弹出了。
...
![img](E:\coder\09_OneNote\image\ab3646b742d946659e4bce34d5af6751tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp) 可以看到报错在 `foo` 函数，`foo` 函数又是在 `bar` 函数中调用的。当使用递归时，因为栈可存放的函数是有**限制**的，一旦存放了过多的函数且没有得到释放的话，就会出现爆栈的问题
...
![img](E:\coder\09_OneNote\image\cac7e59102dc4c3a9efa53aecd64bf9dtplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)

### 9. Node 中的 Event Loop 和浏览器中的有什么区别？process.nextTick 执行顺序？

Node 中的 Event Loop 和浏览器中的是完全不相同的东西。

Node 的 Event Loop 分为 6 个阶段，它们会按照**顺序**反复运行。每当进入某一个阶段的时候，都会从对应的回调队列中取出函数去执行。当队列为空或者执行的回调函数数量到达系统设定的阈值，就会进入下一阶段。 ![img](E:\coder\09_OneNote\image\ed6d249f54e242fe9dc267f03fa156d3tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
...
上面都是 macrotask 的执行情况，对于 microtask 来说，它会在以上每个阶段完成前**清空** microtask 队列，下图中的 Tick 就代表了 microtask ![img](E:\coder\09_OneNote\image\8ea27044a0124facaab3e4a8da89d709tplv-k3u1fbpfcp-zoom-in-crop-mark1512000.webp)
